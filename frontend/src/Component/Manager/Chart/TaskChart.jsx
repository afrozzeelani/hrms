...
const TaskChart = () => {
  ...
  const taskStatusCounts = {
    ...
    Completed: tasks.filter((task) => task.status === "Completed" && task.managerEmail === email).length,
    Rejected: tasks.filter((task) => task.status === "Rejected" && task.managerEmail === email).length,
    canceled: tasks.filter((task) => task.status === "Cancelled" && task.managerEmail === email).length,
    Active: tasks.filter((task) => task.status === "Pending" && task.managerEmail === email).length,
    Overdue: tasks.filter(
      (task) =>
        task.status === "Pending" && task.managerEmail === email && calculateRemainingTime(task.endDate).delay
    ).length,
    Ontime: tasks.filter(
      (task) =>
        task.status === "Pending" && task.managerEmail === email && !calculateRemainingTime(task.endDate).delay
